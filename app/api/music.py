...
import app.services.jiosaavn.helper as helper
...
def search_for_song(query: str, lyrics: bool = False, songdata: bool = True):
    ...
    try:
        ...
        # Only fetch details for first song to avoid timeout
        if song_response and len(song_response) > 0:
            first_song = song_response[0]
        ...
        print(f"⏱️ JioSaavn search timed out for query: {query}")
        return []
    except requests.exceptions.RequestException as e:
        print(f"❌ JioSaavn request error for query '{query}': {e}")
        return []
    except (json.JSONDecodeError, KeyError) as e:
        print(f"❌ JioSaavn response parsing error for query '{query}': {e}")
        return []
...
def get_song(song_id: str, lyrics: bool = False):
    ...
    try:
        ...
        song_response = requests.get(song_url, timeout=10).text.encode().decode('unicode-escape')
        song_response = json.loads(song_response)
        song_data = helper.format_song(song_response[song_id], lyrics)
        if song_data:
            return song_data
    except requests.exceptions.Timeout:
        print(f"⏱️ JioSaavn song details timed out for ID: {song_id}")
        return None
    except requests.exceptions.RequestException as e:
        print(f"❌ JioSaavn request error for song ID '{song_id}': {e}")
        return None
    except Exception as e:
        print(f"❌ Error getting song details for ID '{song_id}': {e}")
        return None
    return None
...
def get_album(album_id: str, lyrics: bool = False):
    ...
    try:
        response = requests.get(endpoints.album_details_base_url + album_id, timeout=15)
...
def get_playlist(list_id: str, lyrics: bool = False):
    ...
    try:
        response = requests.get(endpoints.playlist_details_base_url + list_id, timeout=15)
        if response.status_code == 200:
            songs_json = response.text.encode().decode('unicode-escape')
            songs_json = json.loads(songs_json)
